...
|-------|-------|
| Status | Draft |
| Related | [24-tools-registry.md](./24-tools-registry.md), [11-quality-and-finalization.md](./11-quality-and-finalization.md), [00-overview.md](./00-overview.md) |
...
> **Back pressure** is the org's name for the requirements practice seen from the generation side: rather than only checking work after the fact, the in-scope requirement statements are surfaced *before* the work is done, so the doer is constrained up front. The registry pushes back on generation — it is the pressure that shapes work before it exists, the complement of the gate that checks it afterward.

Back pressure is **not an** infrastructure traffic mechanism — not queue throttling, rate limiting, or transport-level flow control. It is the requirements-as-constraint discipline: the statement side of a requirement pushing back on generation, never a runtime throttle over a data stream.
...
- **Git / security requirements** are pulled from the memo spec's git-security area ([16-git-security-versioning.md](./16-git-security-versioning.md)) and the core `git/*` skills.
