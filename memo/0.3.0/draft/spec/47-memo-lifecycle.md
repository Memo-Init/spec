...
|---|---|
| Status | Draft |
| Depends on | [./11-quality-and-finalization.md](./11-quality-and-finalization.md), [./12-rollout.md](./12-rollout.md) |
| Related | [./02-memo-sop-entrypoint.md](./02-memo-sop-entrypoint.md), [./13-orchestration.md](./13-orchestration.md), [./38-stage-model.md](./38-stage-model.md), [./26-memo-history.md](./26-memo-history.md), [./31-goals.md](./31-goals.md) |
...
The lifecycle sits **above** the phase-level state of a rollout. It is not the phase state machine of [13-orchestration.md](./13-orchestration.md) (which tracks PRD-generation and execution inside one rollout) and it does not replace it. It is the memo's own arc, from the empty slot before authoring to the terminal state after the push — the layer the four-stage process model of [38-stage-model.md](./38-stage-model.md) describes narratively, made into queryable data.
...
- **The phase state files** (`state.json` and its siblings, [13-orchestration.md](./13-orchestration.md)) track progress **inside** a rollout, at PRD/phase granularity, and serve crash-recovery. Their vocabulary (`pending`, `in_progress`, `completed`, `failed`) is the phase level. The lifecycle is the memo level above them; it does not read or replace them. A `rollout` lifecycle state says the rollout is running — the phase files say how far.
- **The chronicle** ([26-memo-history.md](./26-memo-history.md)) is the narrated story of a memo. It explains; it does not carry a queryable state field. The lifecycle carries the state; the chronicle carries the reasons.
...
- [11-quality-and-finalization.md](./11-quality-and-finalization.md) — finalization is the single binary verdict that produces the `finalisiert-research` or `finalisiert-implementation` transition; there is no conditional intermediate verdict.
- [12-rollout.md](./12-rollout.md) — the rollout runs only from `finalisiert-implementation` (the hard rollout-start gate); its own phase state lives one level below this lifecycle.
- [13-orchestration.md](./13-orchestration.md) — the phase state files are the level below: PRD/phase progress and crash-recovery inside one rollout, not the memo-level arc.
- [38-stage-model.md](./38-stage-model.md) — the four process-end stages (landing, merge, push) are the narrative of the `gelandet → gemerged → abgeschlossen` tail; this chapter makes those stages queryable state.
- [31-goals.md](./31-goals.md) — the goal/maintenance scoring layer is the register→deterministic-CLI-write→board pattern this lifecycle mirrors on the memo level.
