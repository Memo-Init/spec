...
|-------|-------|
| Status | Draft |
| Depends on | [./12-rollout.md](./12-rollout.md), [./27-landing-the-plane.md](./27-landing-the-plane.md) |
| Related | [./12-rollout.md](./12-rollout.md), [./27-landing-the-plane.md](./27-landing-the-plane.md), [./13-orchestration.md](./13-orchestration.md), [./17-git-workflow-and-ids.md](./17-git-workflow-and-ids.md), [./00-overview.md](./00-overview.md) |
...
**Stage 1 — Rollout.** The autonomous half: Generate produces the work orders, Execute implements them across phases, Evaluate checks memo conformity. The stage ends on a green Evaluate. Its full specification is [12-rollout.md](./12-rollout.md). A green Evaluate ends the rollout — it does not end the memo.

**Stage 2 — Landing the Plane.** Landing renders the end-state legible and resumable: it cleans worktrees, names open ends honestly (including a `verdict: OPEN` where that is the truth), writes the chronicle entry and the handover, and leaves the workspace in a state a fresh context can pick up the next morning without questions. Its full specification is [27-landing-the-plane.md](./27-landing-the-plane.md). **Landing is always mandatory** — it is never skipped, regardless of whether the merge stages run immediately or later, because a thread left without a landing is a thread the next session has to reconstruct.

**Stage 3 — Merge Preparation.** With the workspace landed, the branches are named by their memo ID (see [17-git-workflow-and-ids.md](./17-git-workflow-and-ids.md)), the system performs the **local merge up to `main`** deterministically, and the commits are prepared and presented. The stage ends when the result is clean and conflict-free — push-ready, with nothing left to resolve before release.
...
- **Fresh context, guaranteed.** Review runs in a **new context** — a deliberate clearing after the autonomous rollout span, the mirror image of the reset that already happens *before* landing ([27-landing-the-plane.md](./27-landing-the-plane.md)). Judging the whole run inside the still-full rollout context is what a clean cut is meant to prevent: a fresh reader weighs the result, not a contaminated memory of what the work intended.
...
- **The report stays the single render.** The three-area review report is authored once, at landing ([27-landing-the-plane.md](./27-landing-the-plane.md)); this section references it and does not restate it. The **REVIEW bookend** above renders that report at the close of landing; the Review work mode is the coarse arc under which the reading happens. One report, one render — the bookend and the work mode are two views of the same review, not two review mechanisms.
- **Lifecycle state is owned elsewhere.** Review reads and advances the memo's lifecycle state — the transition into a closing state — but does not define that vocabulary; the lifecycle enum is owned by [47-memo-lifecycle.md](./47-memo-lifecycle.md), which Review references rather than rebuilds.
...
- [02-memo-sop-entrypoint.md](./02-memo-sop-entrypoint.md) — the three arcs of a memo's life (Create, Rollout, Review) the work-mode triptych is the coarse lens over.
- [47-memo-lifecycle.md](./47-memo-lifecycle.md) — the memo lifecycle state the Review work mode reads and advances but does not define.
