...
Every **reference table** — a table whose rows are addressed and discussed across revisions — carries a mandatory **`Nr` (id) column**. The ids are **append-only**: a row keeps its id for the life of the memo, ids are **never renumbered and never reused**, and a new row takes **max + 1**. Deleting a row retires its id; it does not free the number.

Each reference-table **type** has a **prefix**, so an id is unambiguous across the whole memo:
...
A table whose type is not in this register **declares its own prefix** in place. The display form may read "Annotation 4"; the stable id is the prefixed, zero-padded form (`ANM-004`) — a bare "A4" is ambiguous where a memo also clusters rows such as `A1` next to `F4`, so the prefix is required, not optional. Revision-stable numbering is the precondition for annotating a table's rows across revisions: an id that never moves is what a later annotation points at.
