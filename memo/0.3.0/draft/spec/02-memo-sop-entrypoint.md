...
| 4 | Revision loop | A revision enters the Generate → Execute → Evaluate loop. | [07-revisions-and-questions.md](./07-revisions-and-questions.md) |
...
| 6 | Finalization → rollout | The finalized memo drives a single-memo rollout. | [11-quality-and-finalization.md](./11-quality-and-finalization.md), [12-rollout.md](./12-rollout.md) |

A finalized memo is carried into execution through the **rollout** — a single finalized memo worked straight through its phases in one autonomous run, entered through the public `memo-rollout` entry point. A rollout is, in effect, **a plan with one memo**: the multi-memo **plan layer** that once sat above the rollout is **deprecated**. It was never used — no plans were ever created, while the single-memo rollout carried every execution — so the rollout, not the plan, is the real execution machine. The plan layer's one non-duplicative capability, the budget wake-up, is **lifted down into the rollout** ([12-rollout.md](./12-rollout.md)). This deprecation does not change the six steps above; it settles how step 6 is carried out — always as a single-memo rollout.

Across its life a memo runs in exactly three **arcs**: **Create** — the interactive first arc, spanning steps 1–5 and finalization, where the developer's judgement is spent — **Rollout** ([12-rollout.md](./12-rollout.md)), the autonomous second arc that carries the finalized memo through step 6 — and **Review** ([38-stage-model.md](./38-stage-model.md)), the fresh-context third arc that reads the run's result after the autonomous span. An *arc* names one of the three parts of a memo's life; it is deliberately distinct from a *step* (one of the six above), a *stage* (one of the four end-stages), and a *phase* (a unit inside a rollout). It is also not the per-transcript *context mode* of [03-input-paths.md](./03-input-paths.md), which chooses empty vs in-thread context.

> **Four counts, four nouns — do not conflate them.** The memo path's **six steps** here are distinct from the **three arcs** a memo's life runs in (Create, Rollout and Review, above), from the **four stages** of the end-of-process model ([38-stage-model.md](./38-stage-model.md)), and from the **eight principles** of the CLI doctrine. Four separate enumerations, each bound to its own noun — *steps* (the memo path), *arcs* (a memo's three-part life), *stages* (the process end), *principles* (the CLI) — and "stage" is reserved for the four-stage model alone, just as "phase" is reserved for the rollout's phases.
...
- [01-philosophy.md](./01-philosophy.md) — why the SOP defines the guardrails.
- [03-input-paths.md](./03-input-paths.md) — the four transcript types the SOP routes on.
- [11-quality-and-finalization.md](./11-quality-and-finalization.md) — the developer-triggered finalize verb.
- [13-orchestration.md](./13-orchestration.md) — state and crash recovery behind re-entry.
