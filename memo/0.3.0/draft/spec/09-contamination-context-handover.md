...
|-------|-------|
| Status | Draft |
| Depends on | [08-phases-and-prds.md](./08-phases-and-prds.md) |
| Related | [10-proactive-research.md](./10-proactive-research.md), [11-quality-and-finalization.md](./11-quality-and-finalization.md), [13-orchestration.md](./13-orchestration.md), [00-overview.md](./00-overview.md) |
...
## Contamination vs. Prompt Injection

> Contamination is the **umbrella term** for a context taking on content that mis-weights the output. Prompt injection — externally, maliciously supplied instructions — is the **malicious special case**, not the general rule. The common case is **unconscious self-contamination**: the working context quietly loads material that was never meant as an instruction (an already-generated page in the same topic, a stale note, a half-formed draft) and lets it steer the result. The literature's default name, *prompt injection*, is too narrow because it implies an outside attacker; the org uses *contamination* as the broader, first-class term and treats prompt injection as one bounded subtype of it.

The six-signal detector below and the *Contaminated File* class are the machinery that catches every contamination, whatever its origin: a malicious injection and an unconscious self-contamination trip the same signals and are read the same way. This section only places prompt injection inside the broader term; it does not add a second detector or redefine the signals.
...
- An evaluator (PRD evaluator, phase evaluator) MUST run in a fresh context with no carry-over from the implementation process. It receives only the artifact under review (the PRD document plus the produced files), never the conversation that produced them. See [13-orchestration.md](./13-orchestration.md).
...
### Create is interactive — abort and restart are allowed

Because the **Create** arc — authoring and the revision loop before finalization (defined in [02-memo-sop-entrypoint.md](./02-memo-sop-entrypoint.md)) — is interactive and inexpensive to redo, **abort and a fresh restart are always allowed**. When a Create session drifts (an overflowing context, an assumption that proved wrong), the sound response is to abort and begin again in a clean context rather than push a degraded memo toward finalization. This is the empty-context rule above applied to the interactive arc: the developer's judgement is spent during Create, and starting over costs little next to carrying contamination forward into the autonomous **Rollout** arc.
...
> Trigger: a handed-over `_grading` list was once taken at face value (the take-at-face-value anti-pattern) and propagated as truth. This section makes that a normative failure and gives the spec→skill lint (chapter 10) a stable anchor to key on.
...
The word **quarantine** is already claimed as a **noun / place** by the session spec: the fetch/egress gate is an opt-in, fail-open **"egress quarantine"** (`spec/session/0.3.0/draft/spec/03-recovery.md:48-50`). To avoid a naming collision across the two specs, chapter `09` uses **"quarantine" only as a verb** — the *quarantine-read*, i.e. the sub-agent isolation of a contaminated file — and **never** as the name of a store. The place a Snag is filed is the **Snag-Store**; there is no store named "quarantine".
