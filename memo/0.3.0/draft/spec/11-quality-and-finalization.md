...
> Context quality does not fall off a cliff between a "smart" and a "dumb" zone — a 0/1 split is a simplification. It follows a **curve**: much capacity at the start, flattening relatively quickly, and running out flat toward the back. Where exactly the curve bends is **content-dependent** — the same token count degrades differently depending on what fills the window — so no single hard number is correct for every session. The smart-zone and hard-cap fractions for PRD sizing ([08-phases-and-prds.md](./08-phases-and-prds.md)) are points read off this curve, and the context-rot decay ([09-contamination-context-handover.md](./09-contamination-context-handover.md)) is the curve seen as quality loss.
>
...
The finalization-gate rules above are authored **prose-first** as declarative requirements (the prose-first guard, [35-memo-authoring.md](./35-memo-authoring.md) and [23-requirements.md](./23-requirements.md)): each rule's `statement` faces generation and its `check` faces the finalization gate, resolving to a ternary `PASS` / `BLOCKED` / `INCONCLUSIVE`. The blocks below are the machine-readable source the requirement store is **harvested** from — the gate's own conformity as the worked example of the requirement model. These are the same checks the gate set above runs; this section is their data-backed form, not a second standard.
...
- [23-requirements.md](./23-requirements.md) — the requirement model behind requirement-derived gates, including the back pressure a requirement statement exerts on generation.
