...
The rule: a small problem that threatens to entangle the run and endanger the core memo work is set aside as a **Snag** — not solved on the spot, not raised as a `[NOTSTOPP]`, not raised as a `[BLOCKER]`, and never a reason to stop the run. The Snag is carried to the landing review and presented there ([27-landing-the-plane.md](/specification/landing-the-plane/)); the user decides what becomes of it. It is set aside, named, and shown — never silently dropped, and never allowed to derail the run.
...
## C11 — Restart Instead of Steering a Derailed Trajectory

A long-running session is a single autoregressive trajectory: every step is conditioned on the history that came before it. When that history has visibly derailed — a wrong direction was committed to, the context filled with contradictions or false premises — the tempting move is to steer the same session back on course in place. But in-place correction fights the very history it is conditioned on, and each further step re-conditions on the corrupted trace, compounding the error rather than clearing it.

The rule: when a running session has visibly derailed, restart from a clean context rather than steering the same session further in place. A fresh start conditions only on the durable artifacts — the finalized memo, the state files, the committed code — not on the derailed trace. This is the session-scale companion to fresh-context-per-agent work ([13-orchestration.md](/specification/orchestration/)): the same reason a sub-task runs in its own clean context is why a derailed session is better relaunched than nursed along. The session tree ([14-agents-skills-tasks.md](/specification/agents-skills-tasks/)) is what makes "where did it go wrong" answerable after a restart, so relaunching is a navigable move rather than a loss of work: the branch point is recoverable, and the clean context resumes from it.

## C12 — Hold the Task System, Especially Late

Task discipline is easy to keep at the start of a run and easy to drop once momentum builds. The failure this guards against is the late-run lapse: as a run grows long and attention degrades toward the end, the agent quietly stops tracking its work, so the final steps go unrecorded and unverified — the precise point where a missed step is most costly and least visible.

The rule: use the task system continuously across a run, and most deliberately in its late stages. Task tracking is not an opening formality that can be shed once work is underway; a run's attention is finite and thins toward the end, which is exactly when an untracked step slips through unnoticed. Holding the task system late keeps the remaining work explicit and verifiable when it is most likely to be lost. This rule leans on the task primitives and the session tree ([14-agents-skills-tasks.md](/specification/agents-skills-tasks/)) and on the orchestration model ([13-orchestration.md](/specification/orchestration/)); it is the behavioral counterpart to C6 — work that is not tracked is work whose verification no one is holding.
...
- [27-landing-the-plane.md](/specification/landing-the-plane/) — the landing review that receives the Snag set aside under C10, in its "needs review" area.
