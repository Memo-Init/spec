...
generated_from: "memo/0.3.0/draft/spec/27-landing-the-plane.md"
generator: "scripts/generate-docs-payload.mjs"
edit_warning: "This file is auto-generated. Source: memo/0.3.0/draft/spec/27-landing-the-plane.md."
---
...
- **Proof-of-work archive (before L1, time-critical).** Before the numbered checklist begins, the session proof-of-work is archived — the harness session raw data rolls off after a retention window (~30 days), so the extract MUST run at landing, while this run's sessions are still alive. The tool already exists; landing only *runs* it (`memo session report --memo NNN --subagents --archive`), which writes a no-overwrite snapshot under `<memoDir>/session-report/` (sessions, token totals, wall-time union, marks, gaps — a few KB) with the subagent transcripts folded in. The step is **non-blocking**: a failed extract never stops the landing, it is recorded in the `landing-readiness.json` marker (the `proofOfWork` block) as `skipped`/`failed`, never silently omitted. The archive is the precondition the ledger verifier reads (a snapshot with full marks coverage). The historical backfill over the older memos — whose raw data the retention window will soon delete — is a single user-gated bulk action (`memo session archive-all`, with a read-only `--dry-run` plan) and is never run autonomously.
...
Landing is the **canonical score-refresh point** for the two measured poles of the work: the forward **goal** (the bow, [31-goals.md](/specification/goals/)) and its backward twin **maintenance** (the stern, [33-maintenance.md](/specification/maintenance/)). This is the L6 step of the checklist above. Two reasons make the close of the run the right moment, not the start of the next rollout:
...
- **The context is already fresh.** Scoring both poles carries a hard rule: it is **never** done in the working session, always in a fresh context (the *how*, specified in [31-goals.md](/specification/goals/) and [33-maintenance.md](/specification/maintenance/)). Landing already runs after a context reset (see "Why Landing Is Its Own Step" above), so the fresh reader the score requires is exactly the reader landing already is. The two constraints — *fresh context* and *at landing* — meet here.

Mechanically, L6 spawns `memo goal score-all` and `memo maintenance score-all` (one agent per goal and per repo — the skills fan out per item themselves), captures the resulting boards into the `landing-readiness.json` marker under `L6_scores`, and runs before the L7 chronicle entry. The board mechanics — the strict score object, the second deterministic axis, and the `releaseReady` flag a pre-rollout health check later reads — are **not** restated here; they live in [31-goals.md](/specification/goals/) and [33-maintenance.md](/specification/maintenance/). This chapter fixes only the *when*: the refresh happens at landing.
...
The REVIEW is the back bookend of the process (see [38-stage-model.md](/specification/stage-model/)): the named user touchpoint that gives shape to the interaction model's `U3` "review result" node ([21-human-computer-interaction.md](/specification/human-computer-interaction/)). It is not a new channel — it is the fully-formed expression of the fourth communication point, the bundled hand-back at landing. Where the front bookend is the memo-init question round the user answers *before* the autonomous span, the review report is what the user reads *after* it, and it is rendered on the reply channel (R1) as markdown in the reply, never as a raw command dump ([21-human-computer-interaction.md](/specification/human-computer-interaction/)). The report has exactly **three areas**:

- **(a) The review folder table.** One row per folder or git repo, carried by four columns: `Folder/Git-Repo | inward/outward | fully checked? | anomalies`. The `inward/outward` column is the repository's **facing** — the same `facing` axis Rule C1 uses to decide issue-versus-memo-ID orientation ([17-git-workflow-and-ids.md](/specification/git-workflow-and-ids/)) — recorded here so the review shows, per repo, its direction, whether it was checked in full, and any neutral findings. This table is the review lens over the same repositories the WHAT WAS DONE table records: WHAT WAS DONE says *what changed*, the review folder table says *whether it was checked and what stood out*.
...
- [17-git-workflow-and-ids.md](/specification/git-workflow-and-ids/) — Rule C1 and the `inward/outward` facing axis the review folder table records per repo.
- [31-goals.md](/specification/goals/) — the forward goal pole re-scored at the L6 landing step (`memo goal score-all`, fresh context).
- [33-maintenance.md](/specification/maintenance/) — the backward maintenance twin re-scored at the same L6 step (`memo maintenance score-all`, fresh context).
- [21-human-computer-interaction.md](/specification/human-computer-interaction/) — the interaction model whose `U3` "review result" node the review report gives shape to, and the R1 reply-channel rule it renders on.
- [29-behavioral-guardrails.md](/specification/behavioral-guardrails/) — guardrail C10 (set a Snag aside), whose set-aside items appear in the review report's "needs review" area.
