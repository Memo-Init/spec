...
|-------|-------|
| Status | Draft |
| Depends on | [08-phases-and-prds.md](./08-phases-and-prds.md), [23-requirements.md](./23-requirements.md), [25-strands.md](./25-strands.md) |
| Related | [00-overview.md](./00-overview.md), [04-input-pipeline.md](./04-input-pipeline.md), [06-memo-structure.md](./06-memo-structure.md), [07-revisions-and-questions.md](./07-revisions-and-questions.md), [24-tools-registry.md](./24-tools-registry.md) |
...
> The canonical decomposition chain is **Topic → Block → PRD** (see [08-phases-and-prds.md](./08-phases-and-prds.md)): a block gathers one or more topics, and PRDs are derived from it. Phase and Strand are the **ordering/emergence layer** over the derived PRDs — PRDs are sequenced into phases, and a strand is the emergent dependency closure over phases ([25-strands.md](./25-strands.md)) — not extra links in the derivation. A requirement is likewise **cross-cutting**: a check that reaches work through its PRDs ([23-requirements.md](./23-requirements.md)). A block is a structure node identified by `B\d{3}`, not a synonym for a context block.
...
Related: [06-memo-structure.md](./06-memo-structure.md) (directory layout, local guarantee), [00-overview.md](./00-overview.md) (what a memo is); related primitives: [Revision](#revision), [Topic](#topic), [Block](#block), [Goal](#goal).
...
Related: [07-revisions-and-questions.md](./07-revisions-and-questions.md) (revision structure, question format), [20-flow-full-vs-update-revisions.md](./20-flow-full-vs-update-revisions.md) (Full vs. Update); related primitives: [Memo](#memo), [Topic](#topic).

## Topic

A **topic** is an atomic point extracted from the input during the five-step input pipeline. Topic extraction MUST produce a **complete** list of every point in the input — nothing is too small to omit — and that list then serves as a checklist during writing. Topics are **memo-scoped**: they belong to one memo and are tracked in that memo's topic store (`<memoDir>/_topics/`), with a finalization coverage gate that distinguishes initial topics from those added in later revisions and a HARD-FAIL block-coverage gate that binds every registered topic to at least one block ([11-quality-and-finalization.md](./11-quality-and-finalization.md)). A topic is **captured in a block** when the finalized memo is decomposed, and PRDs are derived from that block; it is the head of the canonical decomposition chain **Topic → Block → PRD**.

Related: [04-input-pipeline.md](./04-input-pipeline.md) (Step 3 topic extraction), [08-phases-and-prds.md](./08-phases-and-prds.md) (topics → blocks → PRDs → phases); related primitives: [Block](#block), [Phase](#phase), [PRD](#prd), [Goal](#goal).
...
Related: [08-phases-and-prds.md](./08-phases-and-prds.md) (PRD definition, cap, smart-zone, required-context), [15-prompt-generator.md](./15-prompt-generator.md) (the PRD's first prompt); related primitives: [Phase](#phase), [Block](#block), [Requirement](#requirement), [Tool](#tool).
...
Related: [24-tools-registry.md](./24-tools-registry.md) (registry entries, checklist mechanic, wiki-as-a-tool), [23-requirements.md](./23-requirements.md) (tool-kind checks); related primitives: [Requirement](#requirement), [PRD](#prd), [Strand](#strand).
...
A work item is stored **deterministically as one JSON file per item** under the memo's `_work-items/` folder (`WI-NNN.json`, ids running `WI-001+` per memo, versioned no-overwrite), and is **auto-rendered** into the revisions by a single shared renderer — `memo work-item render` — that reads the enum values *from the data* (it never re-declares them), escapes table pipes, and emits an autogenerated marker so a hand edit cannot silently diverge from the store. The field set has two levels: the **instance** carried by every item, and the **parent/group** whose shared fields are stored once and inherited at render time rather than duplicated on each item.

**Instance level** — one entry per finding:

| Field | Required | Meaning |
|-------|----------|---------|
| `id` | yes | `WI-NNN`, unique within the memo. |
| `topicId` | yes | `T###` back-reference to an existing topic. The binding is **two-way**: the topic records the item under its own `workItemIds`, so a work item can never float free of a topic. |
| `title` | yes | The finding text. |
| `provenance` | yes | A **structured** array of `{ path, lines }` file:line evidence — never a flat string and never omitted, so every item's acceptance is grep-checkable at the exact location it claims. |
| `status` | default `offen` | One of `offen` · `in-arbeit` · `erledigt`. Changed **only** through the deterministic write path (`set-status`), never by hand. |
| `disposition` | optional | One of `verifizieren` · `kein-defekt` · `behalten` · `dokumentiert` · `auffaellig`. |
| `dispositionNote` | conditional | The reason behind the disposition. **Mandatory** whenever `disposition` is one of the not-fixed values `auffaellig`, `kein-defekt`, or `behalten` — an anomaly, or an "not a defect" judgement, is never waved through silently: it is listed neutrally **and** carries its stated reason. |
| `group` | optional | Pointer to the parent group whose shared fields this item inherits at render time. |

**Parent/group level** — shared fields stored **once**, not repeated on each item:

| Field | Meaning |
|-------|---------|
| `group` | The group name the instances point at. |
| `action` | The shared fix instruction that applies to every item in the group. |
| `changeType` | One of `neu` · `portieren` · `umverdrahten` · `stilllegen` · `migrieren` · `behalten`. |
| `dependsOn` | A list of `WI-NNN` this group's work depends on. |
| `effort` | One of `S` · `M` · `L`. |
| `decisionRef` | A reference to the originating decision the group traces back to. |

**No severity axis.** The core deliberately carries **no `severity` field**. Severity is a grading axis, and the model replaces it with the **neutral** axes `changeType`, `effort`, and `disposition`; where an ordering hint is ever genuinely needed it stays optional and ordering-only, never auto-graded by the agent — consistent with the guardrail that the developer, not the tool, sets priority.

### The CLI contract

Work items are written and read through one CLI branch, `memo work-item`, whose leaves are:

| Leaf | Purpose |
|------|---------|
| `register-group` | Create a parent group with its shared `action` / `changeType` / `effort` / `dependsOn` / `decisionRef`. |
| `register` | Create an instance (requires `topicId` and structured `provenance`). |
| `list` | List the work items of a memo. |
| `set-status` | Move an item's `status` along `offen → in-arbeit → erledigt` — the sole deterministic status write path. |
| `set-disposition` | Set `disposition`, with its mandatory `dispositionNote` for the not-fixed values. |
| `attach-provenance` | Append `{ path, lines }` evidence to an item. |
| `render` | Render the store into the revision table via the one shared renderer (enums read from the data, pipes escaped, autogenerated marker emitted). |
| `topic-gate` | Apply the deterministic topic-solved gate to the evaluator's verdicts: closing every work item of a topic does not prove the topic is solved, so a fresh-context evaluator judges the topic and this leaf turns that judgement into a pass, a reported warning, or (in its hard mode) a block. |
...
| Work item | Finding-level unit of internal work below a topic, bound to the block that gathers it; deterministically stored, auto-rendered, individually rollout-checkable. A granularity, not a primitive. | [08-phases-and-prds.md](./08-phases-and-prds.md) |
...
- [08-phases-and-prds.md](./08-phases-and-prds.md) — the canonical decomposition chain Topic → Block → PRD (sequenced into phases) that this page summarizes.
