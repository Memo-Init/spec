...
|-------|-------|
| Status | Draft |
| Depends on | [./07-revisions-and-questions.md](./07-revisions-and-questions.md), [./21-human-computer-interaction.md](./21-human-computer-interaction.md) |
| Related | [./29-behavioral-guardrails.md](./29-behavioral-guardrails.md), [./31-goals.md](./31-goals.md), [./05-memo-strategies.md](./05-memo-strategies.md) |

The question interface is the primary surface between the user and the agent: most decisions a memo records pass through it. This chapter defines the discipline that surface carries — every option is evaluated and the evaluation is communicated, and the recommendation is made from a **holistic project view** rather than driven by the immediate context. It specifies the evaluation axes (deliberately lenses, not rigid rules), the two channels that present questions, and the deterministic write-format that removes a recurring parser failure. It does not re-define the raw question syntax — that lives in [07-revisions-and-questions.md](./07-revisions-and-questions.md); this chapter adds the reasoning layer above it.
...
Options are scored on **qualitative axes expressed as words**, not as hard numbers — a lightweight, comparable reading without a theatre of invented scores. The axes are **perspectives that help view an implementation from several angles**; they are decision aids, **not rigid prescriptions**. The hard guards stay hard: 100 % feature completeness and no unilateral deferral are not negotiated away by any axis (see [29-behavioral-guardrails.md](./29-behavioral-guardrails.md), C6 and C7).

- **Maintainability** — long-term maintainability from the maintainer's view; no hybrid sprawl that doubles the surface that can drift (see [33-maintenance.md](./33-maintenance.md), [27-landing-the-plane.md](./27-landing-the-plane.md)).
...
- **Defer-Last** — its own axis: any option that defers work (a follow-up memo, a sub-memo, a "later") MUST **name the planning overhead it incurs** explicitly. There is no flat penalty for deferring — a blanket deduction would be unfair (cf. the explicit conflict above) — but the cost is made visible rather than waved through. This axis is the scoring counterpart of guardrail C7 ([29-behavioral-guardrails.md](./29-behavioral-guardrails.md)): deferral is the user's decision, and its price is shown.
...
## The Delegated-Case Exception: a 0–5 Score Matrix

The "words, not hard numbers" rule above has one deliberate **exception**, and it is narrow. When the developer **delegates the decision** — "you decide", "I'm unsure" — or the agent is otherwise **genuinely uncertain**, a recommendation that merely leans to the marginally higher side (a 55-to-45 split the developer could have settled with a coin toss) is a poor answer. In exactly that case the agent switches from words to a **structured score matrix**: it forms **~5 named categories** of its own, scores each option **0–5** against them, weighs the trade-offs each score expresses, and **presents the matrix** rather than a single percentage or a coin-flip verdict.

This is an **exception, not a contradiction** of the lenses-are-words rule. The default stays qualitative: for an ordinary question the axes are read as words, because inventing precise numbers there would be the theatre of invented scores the rule guards against. The matrix earns its numbers only when the developer has handed the decision over and a genuinely close call needs to be **made legible** instead of resolved by a hidden margin. The precedent for scoring named categories on a numeric scale already lives in the specification — the weighted, named **dimensions** of the grading head ([23-requirements.md](./23-requirements.md), The Grading Model) — and the delegated-case matrix borrows that shape for a single question.

The exception is **behavior-first**. It adds **no new `questions-json` schema field** and stores **no hard number in the option record**: the matrix is produced and shown on demand as a reasoning aid, and what is recorded remains the developer's decision — or their explicit delegation. It sharpens the recommendation in the one case where words alone underserve the developer; it never becomes a standing numeric scoring layer over every question.
...
The parser injects a **third** standing sibling alongside `custom` and `topic`: **`reframe`**. Where `custom` ("reject" / keep working) and `topic` ("skip via the topic") are ways to decline or defer *this* question, `reframe` says the question **should not be answered as posed at all** — it rests on a **false premise**. Choosing it signals that the correct next move is to **re-formulate the question**, not to pick one of the options as they stand.
...
- [07-revisions-and-questions.md](./07-revisions-and-questions.md) — the raw `### F{N}` and `questions-json` format and the authority rule this chapter builds on.
- [21-human-computer-interaction.md](./21-human-computer-interaction.md) — where the user interacts with questions in the viewer; the canonical interaction model.
- [29-behavioral-guardrails.md](./29-behavioral-guardrails.md) — C2 (every question keeps a "keep working" option), C6 (verification before "done"), C7 (deferral is the user's decision); the hard guards the axes never override.
- [31-goals.md](./31-goals.md) — the holistic, intent-level view the recommendation is made from.
- [41-mental-model.md](./41-mental-model.md) — the User Mental Model that biases each recommendation (advisory, never a question-killer) and the source of the answered-by-developer record.
- [05-memo-strategies.md](./05-memo-strategies.md) — the memo strategy the question interface drives.
