# 43. Skill Authoring & Quality

| Field | Value |
|---|---|
| Status | Draft |
| Depends on | [./14-agents-skills-tasks.md](./14-agents-skills-tasks.md), [./02-memo-sop-entrypoint.md](./02-memo-sop-entrypoint.md), [./23-requirements.md](./23-requirements.md) |
| Related | [./33-maintenance.md](./33-maintenance.md), [./11-quality-and-finalization.md](./11-quality-and-finalization.md), [./30-primitives.md](./30-primitives.md) |

A skill is a procedure the system runs again and again, yet for most of the system's life skill quality was the one thing nothing governed. Chapters classify a skill against an agent and a task ([14-agents-skills-tasks.md](./14-agents-skills-tasks.md)); the entrypoint demands that each skill carry a distinct trigger ([02-memo-sop-entrypoint.md](./02-memo-sop-entrypoint.md)); but no chapter ever said how *good* a skill must be, how it is tested, or how its written specification is kept honest. This chapter fills that gap. It governs the **quality** of a skill — its frontmatter contract, the trigger discipline that decides when it fires, the three-dimensional testing standard that proves it works, and the measurable link from every skill back to the chapter that specifies it. The guiding idea is to make skills **visible, testable, and inspectable** rather than trusted on sight: a skill should be ownable, which means its specification dependency is explicit and its behavior is asserted, not assumed.
...
The classification chapter ([14-agents-skills-tasks.md](./14-agents-skills-tasks.md)) decides *what a skill is* against an agent and a task; this chapter governs *how good a skill is*. The two do not overlap, and one consequence of the boundary must be stated honestly: **not every skill belongs to the process specification at all.**
...
- [./02-memo-sop-entrypoint.md](./02-memo-sop-entrypoint.md) — requires a distinct trigger per skill; this chapter supplies the quality measure for that trigger.
- [./23-requirements.md](./23-requirements.md) — the two-sided declare-and-check model the coverage gate mirrors.
- [./33-maintenance.md](./33-maintenance.md) — the board pattern that turns a link into a measured signal, applied here to skill-to-spec coverage.
- [./11-quality-and-finalization.md](./11-quality-and-finalization.md) — the finalization discipline whose quality posture this chapter extends to skills themselves.
- [./30-primitives.md](./30-primitives.md) — the concept map; a skill is the procedure primitive this chapter measures.
