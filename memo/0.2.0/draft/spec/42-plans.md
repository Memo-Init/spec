...
|-------|-------|
| Status | Deprecated |
| Depends on | [./08-phases-and-prds.md](./08-phases-and-prds.md), [./12-rollout.md](./12-rollout.md), [./38-stage-model.md](./38-stage-model.md) |
...
> **Deprecated — superseded by the single-memo rollout.** The multi-memo plan layer specified in this chapter is **deprecated**. It was never used: no plans were ever created, while the single-memo **rollout** ([./12-rollout.md](./12-rollout.md)) carried every execution. The accurate framing is that **a rollout is a plan with one memo** — the rollout already is the autonomous execution machine, and the plan was a primitive layered on top of it that never earned its keep. Going forward, a finalized memo is executed as a single-memo rollout, and no new work builds on the plan layer. The one capability the plan layer held that the rollout did not — the **budget wake-up** — is **lifted into the rollout** (see [./12-rollout.md](./12-rollout.md) and the [Budget Gate](#budget-gate-and-wake-up-marker) section below). The plan skills are **not deleted here**: removing them is a separate, deliberate step whose timing is the developer's call, so this chapter is retained as the record of the retired primitive rather than ripped out.

A **plan** was the primitive that sat above the memo and sequenced the work of one or more finalized memos into a single ordered run; it is now **deprecated** (see the banner above), and the description below documents the retired design. A memo is the highest authority over its own rollout, but a memo's rollout knows nothing about the memo next to it; when several finalized memos must be worked off together — interleaved, or with one memo's phase depending on another's — the plan was meant to be the structure that carried them as one. It did not replace the per-memo stage model; it nested it, applying the same four stages to each memo it carried while it recorded, across all of them, which phase runs next and which have reached the push gate. This chapter specifies the plan store on disk, the `plan.md` skeleton and the seven mandatory phase fields, the `plan-status.json` schema, next-phase resolution under memo authority, the budget gate and wake-up marker (now lifted to the rollout), crash recovery, archival, conformance evaluation, the plan-stop verb, checkbox-to-status synchronization, unplanned-memo detection, the plan-done-vs-memo-done distinction, and the coexistence of a standalone rollout with a rollout inside a plan context.
...
> **Lifted into the rollout.** This is the one plan-layer capability the single-memo rollout did not already have, and it is **lifted out of the plan layer into the rollout** ([./12-rollout.md](./12-rollout.md)), where it couples to the five-hour cron mechanic. The description below is retained as the origin record of the mechanism; the live home of the wake-up is now the rollout.
...
Scheduling a wake-up is, **in this plan-era design, a marker rather than a cron job**: the plan system had no scheduler, so it could not wake itself and could only reliably refuse to start too early. This is precisely the point the lifted rollout version **reconciles** — in the rollout the wake-up is now backed by a real scheduled **cron self-restart** ([./12-rollout.md](./12-rollout.md)), so it is no longer merely a passive marker; the description below is retained as the origin record of the marker-only mechanism. A wake-up writes `budget.scheduleWakeup = { phaseId, wakeupIso, scheduledAt }`, sets the phase to `blocked` with a block reason, and breaks. On the next plan-execute, if the current time has reached `wakeupIso` the phase is reset to `pending` and the marker cleared; otherwise the wake-up message is re-emitted and the run breaks again. The only legitimate source of a real budget snapshot is the dedicated paste step at finalization; third-party usage tooling is forbidden.
...
- [./30-primitives.md](./30-primitives.md) — the eleven primitives, of which the plan is the one that spans many memos (now deprecated).
- [./08-phases-and-prds.md](./08-phases-and-prds.md) — phases, PRDs, and the `## Phase-Hints` table a plan reads for next-phase resolution.
- [./12-rollout.md](./12-rollout.md) — the single-memo rollout that **supersedes** this deprecated layer; a rollout is a plan with one memo, and the budget wake-up is lifted there.
- [./38-stage-model.md](./38-stage-model.md) — the four-stage process end the plan nests per carried memo.
- [./13-orchestration.md](./13-orchestration.md) — the state files and orchestration the plan execution drives.
- [./06-memo-structure.md](./06-memo-structure.md) — the memo and revision structure a plan reads to assemble its phases.
- [./09-contamination-context-handover.md](./09-contamination-context-handover.md) — the handover discipline behind plan-stop and re-entry.
