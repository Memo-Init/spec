...
The rollout turns a finalized memo into implemented code. It begins only **after** finalization (memo status "Finalized" or "Conditionally finalized") and only for memos of type Implementation — strategy memos end at finalization. A memo's life runs in exactly two **arcs** — **Create** and **Rollout** (defined in [02-memo-sop-entrypoint.md](./02-memo-sop-entrypoint.md)). Create is the interactive first arc: authoring, the revision loop, and finalization, where the developer's judgement is spent. Rollout, specified in this chapter, is the autonomous second arc: after the user confirms, it runs to completion without intermediate stops.
...
- [02-memo-sop-entrypoint.md](./02-memo-sop-entrypoint.md) — the two arcs a memo's life runs in (Create and Rollout); this chapter specifies the Rollout arc.
