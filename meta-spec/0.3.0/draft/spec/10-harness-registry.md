...
|---|---|
| Status | Draft |
| Depends on | [./07-versioning.md](./07-versioning.md), [./08-spec-lifecycle.md](./08-spec-lifecycle.md) |
| Related | [./05-publishing-principle.md](./05-publishing-principle.md), [./00-overview.md](./00-overview.md) |
...
> The **inner harness** is the surface the agent harness itself exposes and the org does **not** control: the tool definitions, the agent/skill/task mechanics, the config surface a harness version carries — the *given*. The **outer harness** is everything the org builds *around* that given: the standing rules and hooks, the local memo convention, the viewer and its channels — the layer the org owns and shapes. The descriptor above captures the inner harness (its tool and config surface); the outer harness is the project layer that runs on top of it. Naming the two separates responsibility cleanly: a behaviour rooted in the inner harness is not ours to change — we build around it, never against it.

---

## Identity — the Harness ID and the Config Pair

A harness is identified by the same shape as the spec reference ID ([./08-spec-lifecycle.md](./08-spec-lifecycle.md), Part A): `<harnessId>@<version>`. The first registered instance is `claude-code@2.1.206` — `harnessId` is the lowercase harness slug, `version` is the harness's own release version (a bare semantic version, no `v` prefix, mirroring [./07-versioning.md](./07-versioning.md)).
...
The three role names — `user`, `orchestrator`, `worker` — are registered as **anchor terms** in the org anchor-term register (the meta-spec anchor-term convention, [./06-conventions-writing.md](./06-conventions-writing.md)), so their canonical labels and negative delimitations are pinned against mis-labelling. The register entries point back at this section as their owning definition; how a *running context* is assigned one of these roles is specified below.

---

## Part C — The Third Drift Boundary (dist to skills)

The spec lifecycle ([./08-spec-lifecycle.md](./08-spec-lifecycle.md)) names two drift boundaries: `draft → dist` (an authored change drifts its dependent chapters) and `dist → website` (the served site drifts from the spec-emitted bundle). The harness registry adds a **third** boundary, **per harness**:
...
- [./08-spec-lifecycle.md](./08-spec-lifecycle.md) — the org-wide spec lifecycle whose two drift boundaries this chapter extends with a third (`dist → skills`).
- [./07-versioning.md](./07-versioning.md) — the bare-semver directory convention and the release strategy harness versioning stays clear of.
- [./00-overview.md](./00-overview.md) — the Meta-Specification entry point and the RFC-2119 conformance anchor this chapter is read under.
