...
|---|---|
| Status | Draft |
| Depends on | [./08-spec-lifecycle.md](./08-spec-lifecycle.md) |
| Related | [./05-publishing-principle.md](./05-publishing-principle.md), [/specification/maintenance/](/specification/maintenance/) |
...
This is also where the spec's reach ends cleanly. A specification has authority over the **outer harness** the org builds and shapes — its conventions, standing rules, and layered mechanics — while the **inner harness** a running agent harness exposes (its given tool and config surface) is captured descriptively by the harness registry ([./10-harness-registry.md](./10-harness-registry.md)), not dictated by spec prose. The spec frames the layer the org owns; it does not legislate the given surface it merely records.
...
- [./08-spec-lifecycle.md](./08-spec-lifecycle.md) — the lifecycle and versioning gate through which a change to a spec's frame is made deliberate rather than casual.
- [./05-publishing-principle.md](./05-publishing-principle.md) — what of a spec and its implementation is published versus kept private.
