...
Beside its structural head ([02-per-chapter-format.md](./02-per-chapter-format.md)), each specification family declares its **responsibility** in one normative shape, so that a reader — and a review — sees without inference what a family owns, what it hands off, and what it leans on. The shape has three fields:
...
- [./02-per-chapter-format.md](./02-per-chapter-format.md) — the invariant elements every chapter carries, the family head, and the draft-to-dist pipeline.
- [./03-categories.md](./03-categories.md) — the navigation categories a family's manifest declares.
- [./04-bridge-standard.md](./04-bridge-standard.md) — the generated bridge that maps chapters to implementing skills.
- [./05-publishing-principle.md](./05-publishing-principle.md) — what is published (spec + build-plan) versus kept private (skills + interpretation).
- [./06-conventions-writing.md](./06-conventions-writing.md) — how conventions are written and registered as policy blocks.
