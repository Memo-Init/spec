# Bridge — 06-namespace-registry

| Field | Value |
|---|---|
| Family | session |
| Chapter | [06-namespace-registry](./06-namespace-registry.md) |
| Provenance | `c92c914d2610` |

> **Informative · generated.** One read-projection of the skill-to-spec edge. Do not edit by hand.

<!-- Auto-generated by scripts/generate-bridge.mjs from the skill-to-spec map. -->

## 1. SOP anchor

This chapter is entered through the session SOP: [10-sop](./10-sop.md).

## 2. Public entry points

`session-namespace-registry` _(inferred from primary owners)_ · docs entry `https://memo-init.github.io/session/overview/`

## 3. Required detail pages

[02-enforcement](./02-enforcement.md), [05-config-cascade](./05-config-cascade.md), [07-doctor-init](./07-doctor-init.md), [10-sop](./10-sop.md)

## 4. Implementing skills

| Skill | Role | Purpose |
|---|---|---|
| `session-namespace-registry` | primary | Author and validate the sops[] namespace registry — the registrant blocks that replace the flat skills[] list |

## 5. Grading assignment

No grader assigned yet.

## 6. Gaps roll-up

— none —

## 7. Acknowledged internal tooling (out-of-scope)

— none —

## 8. Provenance

Derived-record hash `c92c914d2610` over the skill-to-spec map. Regenerated on every build.
