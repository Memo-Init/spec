...
description: "Every piece of Node.js code in this organisation is written to one house style. The style is not a matter of taste applied per file; it is a single, deterministic contract so that any module reads..."
spec_version: "0.1.0"
spec_file: "47-node-standards.md"
order: 47
section: "Specification"
normative: true
generated_at: "2026-06-30T11:51:22.091Z"
generated_from: "spec/v0.1.0/47-node-standards.md"
generator: "scripts/generate-docs-payload.mjs"
edit_warning: "This file is auto-generated. Source: spec/v0.1.0/47-node-standards.md."
---


> **Normative.** Conformance language (`MUST`, `MUST NOT`, `SHOULD`) is used as defined in [00-overview.md](/specification/overview/).

Every piece of Node.js code in this organisation is written to one house style. The style is not a matter of taste applied per file; it is a single, deterministic contract so that any module reads like every other module and so that an agent can both **generate** new code and **verify** existing code against the same rules. This chapter states that contract. It is the spec-side home for the *code-patterns* family of conformity rules: the prose below is the normative source, and the structured blocks under [Conformity Requirements](#conformity-requirements) are the machine-readable rules the requirement store is harvested from ([23-requirements.md](/specification/requirements/)).

The rules here apply to all Node.js source in every repo (`when.language: ["node"]`, [23-requirements.md](/specification/requirements/)). They do not apply to other languages, and they are intentionally strict: a deterministic style is what lets help-as-spec and the conformity gates ([22-tree-cli-recommended-way.md](/specification/tree-cli-recommended-way/)) work without a human in the loop.
...
The normative rules above are authored here **prose-first** ([35-memo-authoring.md](/specification/memo-authoring/)): each rule's `statement` faces generation (it shapes the prompt that writes a Node module) and its `check` faces the finalization/push gate (it verifies written code, ternary `PASS` / `BLOCKED` / `INCONCLUSIVE`). Every block is scoped to the `code-patterns` category and carries the `node` tag so the requirement store can match it to Node source ([23-requirements.md](/specification/requirements/) anticipates `when.language: ["node"]`).
...
- [22-tree-cli-recommended-way.md](/specification/tree-cli-recommended-way/) — the internal-CLI build pattern these standards apply to.
- [23-requirements.md](/specification/requirements/) — the requirement model, scope axes, and `when.language` matching that harvest these blocks.
- [43-skill-authoring-and-quality.md](/specification/skill-authoring-and-quality/) — the skills that implement and verify these coding rules.
- [44-repository-and-outward-docs.md](/specification/repository-and-outward-docs/) — repository structure and outward-facing documentation standards.
