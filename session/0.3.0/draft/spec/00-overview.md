...
Progressive Disclosure is more than one family's mechanism — it is the **unifying principle** the three sibling specs (`session`, `workbench`, `memo`) share, the umbrella under which their otherwise separate concerns turn out to be facets of one idea:

- **Enforcement** — the deterministic PreToolUse gate ([02-enforcement.md](./02-enforcement.md)) is disclosure made *conditional*: a door is disclosed by intent, and the gate decides whether it opens.
- **Determinism** — identity, root, and the SOP chain resolved **once** and read from a pin ([08-identity-pin.md](./08-identity-pin.md)) are what keep a just-in-time disclosure *stable*: a capability drawn late still resolves to the same answer it would have at the start.
- **Roles** — the three session types and the harness roles they map to ([01-genesis-root.md](./01-genesis-root.md)) scope disclosure *per thread*: each of the User, Orchestrator, and Worker threads sees only the surface its role discloses.

The Workbench spec applies the very same mechanism to its own skill set — a workbench skill is brought into context only when its intent shows up, not all at once ([workbench/24-skills-scope.md](/workbench/skills-scope/)). The SOP layer shows the same principle once more: the four-part **common denominator** (Setup, Health, Update) is the predictable shape through which each tool's procedure is disclosed and made findable ([11-common-denominator.md](./11-common-denominator.md)). One principle, disclosed differently at each tier.
...
Progressive Disclosure (when a capability becomes *visible*) must not be conflated with the **Precondition-Gate** (whether a capability is *allowed to run*). The driver's-licence rule — `memo-init` may run only once `memo-sop` has been read — is a Precondition-Gate: the deterministic SessionStart / PreToolUse chain (`REQ-SS-*`) specified in [02-enforcement.md](./02-enforcement.md). The two are complementary: disclosure decides *when* a door appears; the gate decides *whether* you may walk through it. Someone who never intends to drive needs no licence — pure disclosure; the licence itself is the gate laid over the disclosure.

Progressive Disclosure is also a stance on *how much* context is right. Context is like **compute that must run friction-free** — the answer is not *less* context but **well-built, non-contradictory, building** context, drawn just-in-time. The dangerous half-truth "less context is better" is rejected: a well-constructed context helps more than it harms. The real hazard is not contradictory information but **doubled** information across the standing rules — the same rule stated twice, drifting apart. The discipline that follows is knowing **when to stop and set a reference instead of repeating**: when a rule already has a home, point at it rather than restating it. That discipline is specified as [No Double Normativity](/spec/conventions-writing/) in the writing conventions and is not restated here.
...
- **SOP** — Standard Operating Procedure: the connecting layer that makes a procedure predictable, and the entry-point mechanism through which a Tool registers, attaches to the session, and becomes findable ([10-sop.md](./10-sop.md)). Not every Tool is an SOP — a catalog tool carries skills but defines no procedure.
...
- **Lineage** — an equivalence class of sessions that form **one line of work** across the `/clear` boundary, keyed by a `lineageId` whose value is the `sessionId` of the **first** session of the line ([18-lineage.md](./18-lineage.md)). It is deliberately **distinct** from the *Genesis Root* (above / [01-genesis-root.md](./01-genesis-root.md)): the Genesis Root is the lowest **tier**, one *per session*; a Lineage spans **many** sessions and is keyed on the first of them. A changed `sessionId` opens a new session but the same lineage carries on — the lineage is the thread the cut does not break.
