...
| Lineage-Log (append-only line edges: `{ sessionId, parentSessionId, lineageId, event, memoId }`) | session (per-place `.session/`) | **list-union** (append-only, never rewritten); memo-**independent**, so a session before any memo id is recordable ([18-lineage.md](./18-lineage.md)) |
...
| Path | Scope | Holds |
|------|-------|-------|
| `.session/` | **per-place** (sibling of `.workbench/`, one per tree) | this location's `config.json` (chain edges, namespaces, registrant blocks) **and** the append-only `lineage.jsonl` line ledger ([18-lineage.md](./18-lineage.md)) — project-level, memo-independent, so a session that runs before any memo id is still recordable |
