...
|-------|--------------|--------|
| **Start** | The session opens and its identity is **pinned**: SessionStart resolves `sessionId`, the session root, the security level (and the model/effort) **once** and caches them. This is the moment the genesis root is established. | [08-identity-pin.md](./08-identity-pin.md) |
| **Work** | The session runs its purpose under the pinned identity: SOPs load by intent (Progressive Disclosure), the PreToolUse gate admits each tool against the pinned state, and the three threads (User / Orchestrator / Worker, below) each act within their bounded role. | [02-enforcement.md](./02-enforcement.md) |
| **End** | The purpose is discharged and the session closes. Because identity was pinned at Start and never re-derived, the session ends against the **same** identity it began with — a clean boundary for the next session to open its own genesis root. | [08-identity-pin.md](./08-identity-pin.md) |
...
| **Subagent-spawn session** | A spawned worker with an empty context, started by an orchestrator for one scoped task. | Its own, **separately bounded** profile — the subagent carve-out ([02-enforcement.md](./02-enforcement.md)). A spawn **MUST NOT** hold more privilege than the orchestrator that started it. |
...
- [03-recovery.md](./03-recovery.md) — the fail-safe guarantees that keep the genesis tier always recoverable.
