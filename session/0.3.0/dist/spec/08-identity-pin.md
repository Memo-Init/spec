...
generated_from: "session/0.3.0/draft/spec/08-identity-pin.md"
generator: "scripts/generate-docs-payload.mjs"
edit_warning: "This file is auto-generated. Source: session/0.3.0/draft/spec/08-identity-pin.md."
---
...
## The Work-Mode-State — idle → Create → Rollout → Review, Immutable In-Session

Beyond identity, model, and effort, a memo the session works on sits in exactly one of four **work modes**, and that mode is part of the session's stable state:

- **idle** — the floor: a session that is in a project but not bound to any memo, or a memo whose entry-point log is still empty. It is the explicit "no mode yet", never a guessed default — the resting value *below* the three memo arcs.
...
- **Review** — the fresh-context evaluation arc: the session is checking the rolled-out result against the memo (the review `memo-rollout-evaluate` runs after a rollout).

The work mode is **not stored as mutable state and is never mutated in place**. It is *derived* from an immutable, append-only entry-point log that the public entry points write to (one marker per fire). Because that log only ever grows, the derivation is **monotonic and one-way** along a fixed precedence **Review > Rollout > Create > idle**: once a higher-arc marker exists for a memo, that mode holds and a later lower-arc marker cannot un-say it — a rollout marker fixes **Rollout** over Create, and a review marker fixes **Review** over Rollout. An empty log derives the **idle** floor — an explicit "no mode yet", never a guessed default. (idle is the no-memo/empty-log floor, *not* a fourth memo arc: the memo's life still has the three arcs Create/Rollout/Review.)
...
### Lineage Inheritance at the Cut

The `sessionId` boundary — the cut — is exactly where the **work-line key `lineageId`** ([01-genesis-root.md](/session/genesis-root/), [18-lineage.md](/session/lineage/)) either continues a line or begins one. The cut here means only the two events above: a `/clear` or a software restart. A `/compact` or a resume keeps the same `sessionId` and is therefore **not** a cut — it needs no lineage handling at all, which keeps the model small.

At the cut, `lineageId` follows one rule, with a no-guess default:

- The new session **inherits** `lineageId` from the predecessor's pin or its handover record — it is **carried forward, never re-derived**. The line survives the new `sessionId` even though `sessionId` itself is fresh.
- When **no** source carries a `lineageId` — the first session of a line — the line is opened on this session: `lineageId := sessionId`. This is the only place a `lineageId` is minted, and it is minted from the session's own id, never guessed.

On the re-pin at the cut, the boundary is recorded as one edge — `{ parentSessionId, sessionId, lineageId, timestamp }` — appended to the project-level lineage log ([18-lineage.md](/session/lineage/), [05-config-cascade.md](/session/config-cascade/)). The append is the only write; the pin itself stays a pure read of the resolved values.

| Requirement | Statement |
|-------------|-----------|
| **REQ-SS-WORKMODE** | A memo's work mode (idle \| Create \| Rollout \| Review) is derived monotonically along the precedence Review > Rollout > Create > idle from the append-only entry-point log and is immutable for the session lifetime; the empty-log/no-memo floor derives idle; only a changed `sessionId` — produced by a `/clear` or a software restart — re-initializes the per-session view. |
| **REQ-SS-LINEAGE** | At a cut (a `/clear` or a software restart), the new session MUST inherit `lineageId` from the predecessor pin/handover — carried forward, never re-derived; absent any source it opens a new line with `lineageId := sessionId`. The cut MUST append one edge `{ parentSessionId, sessionId, lineageId, timestamp }` to the project-level lineage log. A `/compact`/resume keeps the same `sessionId` and is not a cut. |
