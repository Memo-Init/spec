...
generated_from: "session/0.3.0/draft/spec/00-overview.md"
generator: "scripts/generate-docs-payload.mjs"
edit_warning: "This file is auto-generated. Source: session/0.3.0/draft/spec/00-overview.md."
---
...
- **Enforcement** — the deterministic PreToolUse gate ([02-enforcement.md](/session/enforcement/)) is disclosure made *conditional*: a door is disclosed by intent, and the gate decides whether it opens.
- **Determinism** — identity, root, and the SOP chain resolved **once** and read from a pin ([08-identity-pin.md](/session/identity-pin/)) are what keep a just-in-time disclosure *stable*: a capability drawn late still resolves to the same answer it would have at the start.
- **Roles** — the three session types and the harness roles they map to ([01-genesis-root.md](/session/genesis-root/)) scope disclosure *per thread*: each of the User, Orchestrator, and Worker threads sees only the surface its role discloses.

The Workbench spec applies the very same mechanism to its own skill set — a workbench skill is brought into context only when its intent shows up, not all at once ([workbench/24-skills-scope.md](/workbench/skills-scope/)). The SOP layer shows the same principle once more: the four-part **common denominator** (Setup, Health, Update) is the predictable shape through which each tool's procedure is disclosed and made findable ([11-common-denominator.md](/session/common-denominator/)). One principle, disclosed differently at each tier.
