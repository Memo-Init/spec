...
generated_from: "session/0.3.0/draft/spec/05-config-cascade.md"
generator: "scripts/generate-docs-payload.mjs"
edit_warning: "This file is auto-generated. Source: session/0.3.0/draft/spec/05-config-cascade.md."
---


The machine-readable form of the SOP chain (skills, dependency edges, namespace reservations) is a **config** the PreToolUse gate reads. This chapter specifies where that config lives, how it merges across tiers, and the one-time move that relocates it from the workbench tier down to the session tier. The deterministic gate that reads it is specified in [02-enforcement.md](/session/enforcement/); the structure of the registrant blocks inside it is specified in [06-namespace-registry.md](/session/namespace-registry/).

---

## The Entry Point Moves Down a Tier

Today the only active registry is project-scoped — `.workbench/registry.json`, carrying the single live edge REQ-061 (`memo-init → memo-sop`, `when:pre`). Because the workbench is itself just **one session-SOP application** above the genesis root ([01-genesis-root.md](/session/genesis-root/)), the config that anchors the chain belongs at the **session tier**, not the workbench tier.
...
| `security` | the resolved **trust level**; a project may never self-elevate it (monotonicity, [01-genesis-root.md](/session/genesis-root/)). |
| `sops[]` | the **registrant blocks** — one per reserved **namespace**. Each block names its `owner` and `tier`, reserves a namespace, and declares the `cli` it ships, the `folders` it owns, and the `skills[]` it contributes ([06-namespace-registry.md](/session/namespace-registry/)). |
| `requirements[]` | the fine, cross-namespace **pre-gate edges** (entry point → skill, with `when: pre`/`post`) the PreToolUse hook evaluates ([02-enforcement.md](/session/enforcement/)). |
| `assertions[]` | the **policy checkpoint rows** — read-receipt requirements (a `requiresGroup` that must be read by the time a named `checkpoint` skill fires, `onMissing:"redirect"`), fed ONLY by policy blocks; the policy-axis counterpart to `requirements[]`, never mixed with it ([06-namespace-registry.md](/session/namespace-registry/)). |
...
| Security / trust level | session | resolved, NOT cascaded — a project may never self-elevate (monotonicity, see [01-genesis-root.md](/session/genesis-root/)) |
...
| `requirements[]` (fine entrypoint→skill pre-gate edges, e.g. REQ-061) | session base + merge | **list-union**; fed ONLY by SOP-instance blocks ([06-namespace-registry.md](/session/namespace-registry/)) |
| `assertions[]` (policy checkpoint read-receipt rows) | session base + merge | **list-union by id**; fed ONLY by policy blocks; never mixed with `requirements[]` ([06-namespace-registry.md](/session/namespace-registry/)) |
| Lineage-Log (append-only line edges: `{ sessionId, parentSessionId, lineageId, event, memoId }`) | session (per-place `.session/`) | **list-union** (append-only, never rewritten); memo-**independent**, so a session before any memo id is recordable ([18-lineage.md](/session/lineage/)) |
...
| Path | Scope | Holds |
|------|-------|-------|
| `.session/` | **per-place** (sibling of `.workbench/`, one per tree) | this location's `config.json` (chain edges, namespaces, registrant blocks) **and** the append-only `lineage.jsonl` line ledger ([18-lineage.md](/session/lineage/)) — project-level, memo-independent, so a session that runs before any memo id is still recordable |
...
- **One-time move via `session init`** ([07-doctor-init.md](/session/doctor-init/), which carries the migration): `init` proposes the relocated config as a **reviewed diff** and never auto-overwrites an existing file (REQ-SS-NOWRITE).
...
## The Consolidated Init Config-Gate (REQ-SS-CONFIGGATE)

The three-tier cascade above answers *how* the tiers combine; a companion **init config-gate** answers *whether the resolved configuration is sound enough to run against*. Where the plain cascade only JSON-parses each tier and rejects a namespace collision, the gate adds two init-time guarantees and folds in a third from the harness tier, so a session starts against a *validated* configuration rather than a merely parseable one. It is a **read-only** check that **reports**: it MAY exit non-zero so a rollout or CI can gate on a hard mismatch, but it never blocks the session and never writes — the same fail-open contract the doctor and canary carry.

The gate has three parts:

- **Tier schema-validity — a real shape check, not just parseable.** Each of the three tiers is validated against the shape its fields declare: a *known* field carried with the wrong JSON type is a schema violation, an absent field is optional (the cascade's "an absent tier contributes nothing"), and an unknown field is allowed so the config grows forward-compatibly. A present tier with broken JSON is a hard failure exactly as a wrong-typed field is. This is the schema half of single-source-by-ownership — a tier that parses but is mis-shaped is caught before the gate merges it.
- **Folder ↔ config reconcile.** The registered-folder vocabulary (the folder registry, [/workbench/folders/](/workbench/folders/)) is reconciled against what the config *declares* — the folders a namespace owns in its `sops[]` block plus any folder the workbench tier adds. A folder the config declares that the registry does not know is **drift** (declared-but-unregistered) and is surfaced as a hard mismatch; a registered name no config block claims is surfaced **informationally** (registered-but-undeclared) and is never a mismatch, because most registered folders are never owned by a config block.
- **Harness-tools presence and model/effort validity — at init, against the descriptor.** The tools the resolved harness descriptor requires ([/meta-spec/harness-registry/](/meta-spec/harness-registry/)) MUST be present — the same required-name and permission-deny validation the rollout pre-flight applies, reused rather than duplicated — and the running session's **model** and **effort** MUST be ones the descriptor supports. Because a descriptor MAY be silent on model/effort, that half **degrades gracefully**: when the descriptor declares no model or effort, or the session declares none to compare, the check is skipped to a warning and never a false block. A declared-both mismatch is likewise a warning, never a hard stop — the session's declared identity is surfaced, not overridden.

The gate's binding property, `REQ-SS-CONFIGGATE`, is that it **never fails closed**: every input it cannot read degrades to a warning, and a hard mismatch (a schema-invalid tier, a namespace collision, a declared-but-unregistered folder, or an absent or fully-denied required tool) is a *reported verdict* a caller may act on, not a refusal of the session. The session's model and effort are read from the session tier (or an explicit override); they default to nothing rather than to a guessed value, so a missing identity is reported honestly rather than silently assumed.
...
([07-doctor-init.md](/session/doctor-init/)) reports a mis-named stage env file with the fix command,
...
- It **MUST NEVER touch `.env`** — the schema check is read-only over filenames; `.env` is
  ...
  doctor's read-only contract ([07-doctor-init.md](/session/doctor-init/): *"never — read-only, prints
...
- At the repo root, only a **`.example.env`** with **dummy** placeholders is committed — never real
  keys.
- The CLI doctrine already forbids reading secrets from flags or env ([04-cli.md](/session/cli/),
...
- [01-genesis-root.md](/session/genesis-root/) — the tier model, identity, and the security level this cascade keeps resolved-not-overridable.
- [06-namespace-registry.md](/session/namespace-registry/) — the structure of the `sops[]` registrant blocks the cascade merges, and `requires[]` vs `requirements[]`.
- [07-doctor-init.md](/session/doctor-init/) — `session init` carries the one-time migration; `session doctor` validates the resolved config.
- [02-enforcement.md](/session/enforcement/) — the PreToolUse hook that reads this config and enforces its edges.
