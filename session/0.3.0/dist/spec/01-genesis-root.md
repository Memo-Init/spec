...
Being the genesis root is *what the session is* structurally; **doing work** is *what the session is for*. A session is the unit within which an agent is given a purpose, pursues it, and finishes — and that work-orientation gives the session a **lifecycle** with three phases, **Start → Work → End**:

| Phase | What happens | Anchor |
|-------|--------------|--------|
| **Start** | The session opens and its identity is **pinned**: SessionStart resolves `sessionId`, the session root, the security level (and the model/effort) **once** and caches them. This is the moment the genesis root is established. | [08-identity-pin.md](/session/identity-pin/) |
| **Work** | The session runs its purpose under the pinned identity: SOPs load by intent (Progressive Disclosure), the PreToolUse gate admits each tool against the pinned state, and the three threads (User / Orchestrator / Worker, below) each act within their bounded role. | [02-enforcement.md](/session/enforcement/) |
| **End** | The purpose is discharged and the session closes. Because identity was pinned at Start and never re-derived, the session ends against the **same** identity it began with — a clean boundary for the next session to open its own genesis root. | [08-identity-pin.md](/session/identity-pin/) |

The lifecycle is **per session**, exactly like the identity it pins: one genesis root is opened at Start and the same one is closed at End. This work-oriented framing sits *on top of* the genesis-root rationale — the session is both the lowest structural tier **and** the span of one unit of work.
...
| `lineageId` (optional work-line key) | `--lineage-id` | `SOP_LINEAGE_ID` | `null` |
...
`lineageId` is the **work-line key** — a third optional protocol on the same footing as `memoId`. Its value is the `sessionId` of the **first** session of a work line, carried forward across the session boundary so a chain of sessions can be recognized as one line of work (specified in [18-lineage.md](/session/lineage/)). It is a **separate field**, never a reuse of `sessionId` (a changed `sessionId` is by definition a new session, [08-identity-pin.md](/session/identity-pin/)), and it is deliberately distinct from the *Genesis Root* tier name above. Like the identity variable, it is **adapter-separated from the start**: the canonical environment name is `SOP_LINEAGE_ID`, and there is intentionally **no** harness alias — the line key is never coupled to a tool-specific variable. When neither flag nor environment provides it, it resolves to `null` with source `"none"`, and the system MUST function without it.
...
This separation is exactly why "session" is disambiguated in the glossary (see the *Three meanings of "session"* entry in [00-overview.md](/session/overview/#glossary)): the OS/runtime session and the harness's own conversation session are distinct from our `.session/` marker, and the adapter is precisely the seam where a harness's session meets ours.
...
**The three threads.** These three session types are the system's **three threads** — the *User thread* (the interactive surface a developer drives), the *Orchestrator thread* (the main loop coordinating a rollout on the user's behalf), and the *Worker thread* (a spawned subagent running one scoped task). Naming them here is **doctrine only**: the *mechanism* already exists and is the ground truth the doctrine binds to — the harness descriptor's `toolContract.roles` (with its `orchestrator`, `worker`, and `user` entries), declared by the harness descriptor schema ([meta-spec/10-harness-registry.md](/spec/harness-registry/)) and realized live by the per-role agent surface the harness emits. Each thread *is* one entry of `toolContract.roles`: the doctrine here fixes each thread's **trust profile**, and `toolContract.roles` fixes its **tool surface** — the spec names the threads, the descriptor is where their per-role tool deltas actually live.

---

## The Per-Session Security Level

The genesis root owns a **per-session security/trust level** that upper layers read rather than redefine. Its value is **anchored by the session type** (above): each of the three types maps to exactly one profile, established at session start. The level is a property of the session as a whole; it is the anchor the Trust Axis of research ([workbench/31-browser-automation.md](/workbench/browser-automation/)) and the egress gate ([03-recovery.md](/session/recovery/), [02-enforcement.md](/session/enforcement/)) refer to when deciding whether untrusted content may enter a context. The level is established at session start and is **not** elevated by a self-label (see above).
...
The chain's machine-readable home is the session config. The registrant blocks that reserve each namespace and the two edge granularities — coarse `requires[]` between SOPs and fine `requirements[]` pre-gate edges — are specified in [06-namespace-registry.md](/session/namespace-registry/), and the `.session/config.json` cascade that stores them is specified in [05-config-cascade.md](/session/config-cascade/). This chapter declares the chain; those two chapters encode it.
...
- [02-enforcement.md](/session/enforcement/) — how the chain is enforced deterministically at PreToolUse.
- [03-recovery.md](/session/recovery/) — the fail-safe guarantees that keep the genesis tier always recoverable.
