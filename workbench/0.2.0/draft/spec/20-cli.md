# 20. CLI Convention — Branch/Leaf

| | |
|---|---|
| Status | Draft |
| Depends on | [00-overview.md](./00-overview.md) |
| Related | [21-environment-scripts.md](./21-environment-scripts.md), [30-wiki.md](./30-wiki.md), [Tree CLI — the recommended way](/specification/tree-cli-recommended-way/) |
...
**Discovery view vs the SOP carrier.** This "single discovery source" claim is scoped to the **workbench tier's discovery surface** — what CLIs, skills, and folders this workbench exposes. It is **not** the authoritative carrier of SOP ownership (namespaces, owners, and gating edges): that single carrier is the session `sops[]` set in `.session/config.json` ([session/05-config-cascade.md](/session/config-cascade/), *Carrier Harmonization*). After the session-tier move-down, `.workbench/registry.json` is a **generated read-projection** of that carrier — the workbench-tier view over it — never a co-equal second source. Discovery reads the projection; registration lives in the one carrier.
...
- [25-validation-overview.md](./25-validation-overview.md) — the validation wayfinder and code index, and the single source of the before/after split where runtime call-validation is registered.
- [21-environment-scripts.md](./21-environment-scripts.md) — the script families that follow the subfolder rule.
- [30-wiki.md](./30-wiki.md) — the About convention that records what a scripts subfolder is for.
