...
| Related | [11-project-structure.md](./11-project-structure.md), [12-folders.md](./12-folders.md), [22-config.md](./22-config.md), [the Session spec's SOP area](/session/sop/) |

The workbench has **two structural levels**: the workbench root, and the projects beneath it. They have different layouts and different jobs, and keeping them distinct is a precondition for everything else in this spec. This chapter draws that boundary; the per-project layout itself is specified in [12-folders.md](./12-folders.md), and the local guarantee that protects both levels in [11-project-structure.md](./11-project-structure.md). Each level is governed by its own thin SOP — a **Root-SOP** and a **Projects-SOP**, both instances of the common SOP standard ([the Session spec's SOP area](/session/sop/)).
...
Every project lives under `projects/{name}/` and carries its **own** structure, scaled to a single project. That per-project structure (which folders are mandatory, which are optional, and what each is for) is specified in [12-folders.md](./12-folders.md).
...
| **Projects-SOP** | the project level — the project's own work | `repos/`, `context/`, `.memo/`, `design/` (the project-level folders, [12-folders.md](./12-folders.md)) | Setup / Health / Update for the **project** scope: the project's memos, repositories, context, and tooling. |
...
Either level MAY carry a local git repository for versioning, but neither carries a **remote**. This remote-confinement is the structural basis of the **local guarantee**, specified in full in [11-project-structure.md](./11-project-structure.md): material outside `repos/` has no remote and so cannot be pushed, so memos, context, and research stay on the developer's machine by construction. Only the repositories under each project's `repos/` carry a remote and are publishable.

---

## Where Policy Lives

Workbench-level policy — the conventions both levels obey — belongs in **this specification** and in the **`.workbench/` configuration** ([22-config.md](./22-config.md)). A `CLAUDE.md` is a **thin pointer**: it tells an agent which SOP to read ("read the workbench-SOP" at the root, "read the Projects-SOP" inside a project), and states the *why* and *when* of that scope. It is **not** the place where conventions are defined or enforced. The same division of responsibility applies as elsewhere in this spec: the spec and the configuration **declare** policy; the machine tier **enforces** it ([22-config.md](./22-config.md)); and `CLAUDE.md` merely **points** at both.
...
|---------|--------------------|
| Folder conventions (which folders exist, mandatory vs optional, at which level) | this spec ([12-folders.md](./12-folders.md)) |
| The `.memo/` internal layout | this spec ([11-project-structure.md](./11-project-structure.md), [17-memo-store.md](./17-memo-store.md)) |
...
| Inward/outward repo routing (rule C1) | the `.workbench/` configuration ([22-config.md](./22-config.md)) |
...
- [11-project-structure.md](./11-project-structure.md) — the local guarantee that protects both levels.
- [12-folders.md](./12-folders.md) — the mandatory and optional folders of a single project.
- [02-sop-entrypoint.md](./02-sop-entrypoint.md) — the workbench-SOP entry point that routes to the Root-SOP or the Projects-SOP.
- [22-config.md](./22-config.md) — the `.workbench/` configuration, where policy is declared.
- [00-overview.md](./00-overview.md) — projects-not-repositories framing.
