...
The Workbench spec is a **standalone sibling** of the core specification, not a subordinate set of chapters under it. The memo-init project authors **four sibling spec families** side by side in its Workshop `spec/` container — the private, namespace-first authoring source — and promotes them to the public `repos/spec` repository on publish; each family carries its own version line. The directories below are given relative to that `spec/` container:

| Family | Directory | Concern |
|--------|-----------|---------|
| **Core specification** (memo) | `memo/0.2.0/draft/spec/` | A single memo's lifecycle — input, revisions, questions, phases, PRDs, rollout, git flow. Reasons about *one memo at a time*. |
| **Workbench** (this spec) | `workbench/0.2.0/draft/spec/` | The environment in which memos live — project layout, CLIs and scripts, configuration, the wiki, trash. Reasons about *the project and the workbench around the memo*. |
| **Session** | `session/0.2.0/draft/spec/` | The session genesis layer; it also carries the **SOP area** — the thin connecting layer that makes the various SOPs predictable (what a SOP contains and where to find it). The SOP, once framed as its own family, now lives here as that area. |
| **Meta-spec** | `spec/0.2.0/draft/spec/` | The specification-of-specifications: the per-chapter format, categories, and publishing rules every family follows, so a new family is added by following a document rather than reverse-engineering the build. |

This mirrors the established multi-family pattern (a single spec container hosting parallel, independently versioned spec families as sibling directories). Keeping these concerns in separate, peer documents lets each evolve at its own pace.
...
- Inside the Workshop `spec/` container the core specification lives under `memo/0.2.0/draft/spec/`; the Workbench spec lives under `workbench/0.2.0/draft/spec/` and carries its own version line.
...
This distinction is what [11-project-structure.md](./11-project-structure.md) makes concrete, and the separation of the workbench root from the projects beneath it is drawn in [10-root-and-projects.md](./10-root-and-projects.md).

The workbench **core** — its folders, CLIs, and conventions — is independent of the memo system: a project can use the layout without any memo ever being written, even though the memo system is the recommended custom folder that normally drives it. The core-vs-custom folder split among the registered folders is drawn in [12-folders.md](./12-folders.md).
...
1. **Folder structures.** Each project under `projects/` is expected to follow the mandatory layout. The workbench audit walks the structure and reports missing or unexpected folders and files. That layout is a **single definition** — the registered-folder registry in [12-folders.md](./12-folders.md) — verified at two moments: its presence and level at **init** (this audit) and the content each folder must hold at **runtime** (the write-time content lint). This audit is the init-time half.
...
These checks are descriptive of the real audit performed at the workbench level; the normative details of the structure being checked live in [11-project-structure.md](./11-project-structure.md), and the health-check scripts that perform them in [21-environment-scripts.md](./21-environment-scripts.md).
...
- **The machine tier** (hook-based enforcement) — a separate future spec (see above and [02-sop-entrypoint.md](./02-sop-entrypoint.md)).
