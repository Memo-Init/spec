...
description: "Deterministic enforcement of workbench policy is delegated to Claude Code hooks. The **PreToolUse enforcement contract** those hooks satisfy — the gate mechanism, its three-state decision, the signal..."
...
generated_from: "workbench/0.3.0/draft/spec/23-hooks-contract.md"
generator: "scripts/generate-docs-payload.mjs"
edit_warning: "This file is auto-generated. Source: workbench/0.3.0/draft/spec/23-hooks-contract.md."
---
...
An entry is `{ folder, pattern, linter, severity }`: which folder and filename pattern the rule covers, which linter validates the content, and whether a failure blocks (`error`) or only warns (`warn`). Each entry is the **binding** of a folder's content contract — the per-folder content requirement a registered name may reserve ([12-folders.md](/workbench/folders/)): the registry declares *that* a folder has a content requirement, and a `folder-lints.json` entry is *how* that requirement is checked at write time. Adding a lint for a newly reserved name is a data change here, not new hook code.
