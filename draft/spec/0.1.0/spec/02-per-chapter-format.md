...
| Related | [./03-categories.md](./03-categories.md), [./04-bridge-standard.md](./04-bridge-standard.md) |
...
The rendered "Implemented by" backlink (which skills implement the chapter) is a derived artifact that lives only in the dist; the source chapter carries this placeholder and never the full block. This authored-versus-derived split is described in [The Bridge Standard](./04-bridge-standard.md); a chapter author leaves the placeholder line in place verbatim and does not hand-edit the backlink.
...
- The **`NN-bridge.md` name is reserved** for the family's generated bridge hub ([The Bridge Standard](./04-bridge-standard.md)). A family has exactly one bridge chapter; the build reuses its existing number so re-runs stay idempotent.
...
| `groups[]` | The navigation categories, specified in [Navigation Categories](./03-categories.md). |
...
| `sidebarMeta` | The derived sidebar `order` + `labels` view (single-sourced from `groups[]`, [Navigation Categories](./03-categories.md)). |
...
A chapter is authored under `draft/<family>/<version>/spec/`, beside a sibling `draft/<family>/<version>/data/` directory that holds the family's authored data inputs — chiefly the `skill-spec-map.json` the bridge is projected from ([The Bridge Standard](./04-bridge-standard.md)). The build publishes from `dist/<family>/<version>/`. The source is content-first prose; the published payload is the source plus generated metadata. The build is deterministic and idempotent — running it twice over an unchanged source produces the same output.
...
- [./00-overview.md](./00-overview.md) — why this meta-specification exists and how it relates to the three system families.
- [./03-categories.md](./03-categories.md) — the `groups[]` navigation categories the family head carries.
- [./04-bridge-standard.md](./04-bridge-standard.md) — the generated bridge chapter and the implemented-by backlink projected onto each chapter.
