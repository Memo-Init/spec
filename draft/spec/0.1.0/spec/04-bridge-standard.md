...
- **The bridge chapter itself is generated.** Its `NN-bridge` file is reshaped on every build from the skill-to-spec map. An author does not edit it; the reserved `NN-bridge` name ([Per-Chapter Format](./02-per-chapter-format.md)) marks it.
...
- [./01-bridge.md](./01-bridge.md) — this family's generated bridge hub, an instance of the standard specified here.
- [./03-categories.md](./03-categories.md) — the navigation categories the bridge hub mirrors in its per-chapter view.
- [./05-publishing-principle.md](./05-publishing-principle.md) — why the gaps roll-up and provenance are withheld: internal interpretation is private.
