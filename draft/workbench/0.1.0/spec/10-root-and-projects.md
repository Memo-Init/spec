...
Neither the workbench root nor any project root is a git repository. This is the structural basis of the **local guarantee**, specified in full in [11-project-structure.md](./11-project-structure.md): material that has no enclosing repository cannot be pushed, so memos, context, and research stay on the developer's machine by construction. Only the repositories under each project's `repos/` are git units.
