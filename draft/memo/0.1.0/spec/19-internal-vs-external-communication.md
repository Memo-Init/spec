...
| Related | [16-git-security-versioning.md](./16-git-security-versioning.md), [17-git-workflow-and-ids.md](./17-git-workflow-and-ids.md), [00-overview.md](./00-overview.md) |
...
- is written for an outside reader, understandable without insider context (see [00-overview.md](./00-overview.md));
- carries no private data — paths, secrets, personal identifiers (see [16-git-security-versioning.md](./16-git-security-versioning.md));
...
- A code comment **MUST NOT** carry internal references — question IDs (see [17-git-workflow-and-ids.md](./17-git-workflow-and-ids.md)), memo references, or process metadata that only resolves inside the working session.
...
- An issue **SHOULD NOT** expose internal process references or working-session pointers. The commit ID (see [17-git-workflow-and-ids.md](./17-git-workflow-and-ids.md)) is the anchor that ties a published artifact back to inward-facing context without leaking that context outward.
...
Beyond private data (paths, secrets, personal identifiers — see [16-git-security-versioning.md](./16-git-security-versioning.md)), an outward-facing artifact **MUST NOT** carry any of the following inward-facing leaks:

- **Pseudo-secrets** — fabricated credential-shaped strings that read as real secrets even when they are not.
- **Internal answer codes** — a question-answer notation such as a bare `F<n>=<choice>` (the *schema* is documented in [17-git-workflow-and-ids.md](./17-git-workflow-and-ids.md), but a concrete inward answer code is a leak in published text).
...
- The **specific** languages are a **project choice**, recorded in the project's own configuration / house rules, not fixed by this specification. A project that writes its memos in one natural language and publishes in another conforms as long as each artifact stays single-language. The consistency check that enforces single-language-per-artifact at finalization is specified in [11-quality-and-finalization.md](./11-quality-and-finalization.md).

---

## The Trust Layer — Communicating to the User

The directions above govern *published artifacts*. There is a third communication surface this chapter also owns: how the **orchestrator speaks to the user** during a run (Protocol A of the three-layer model, see [21-human-computer-interaction.md](./21-human-computer-interaction.md)). When the orchestrator is *also* the user's output channel, the user is easily buried in text and the important thing is lost underneath. The trust layer is the discipline that prevents that.
...
- [00-overview.md](./00-overview.md) — conformance language and the self-explanatory-to-a-stranger requirement.
- [16-git-security-versioning.md](./16-git-security-versioning.md) — the security review that keeps private data out of public artifacts.
- [17-git-workflow-and-ids.md](./17-git-workflow-and-ids.md) — commit messages as outward-facing artifacts, the inward-facing question-ID schema, and the commit ID that anchors the PRD message channel.
- [21-human-computer-interaction.md](./21-human-computer-interaction.md) — the three communication layers and the clean-terminal mechanisms that carry out the Trust-Layer convention.
- [34-question-interface.md](./34-question-interface.md) — the question interface referenced by J11.
