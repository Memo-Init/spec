...
generated_from: "spec/workbench/0.1.0/draft/spec/19-tmp.md"
generator: "scripts/generate-docs-payload.mjs"
edit_warning: "This file is auto-generated. Source: spec/workbench/0.1.0/draft/spec/19-tmp.md."
---


`.tmp/` is the project's **ephemeral scratch** folder: a dot-prefixed, machine-local area for throwaway working files that no part of the project depends on. It is the scratch counterpart to `.trash/` — where `.trash/` holds *recoverable* deletions, `.tmp/` holds files that were never meant to last at all. This page **owns** the `.tmp/` specification; [12-folders.md](/workbench/folders/) owns the durable/scratch tier model, and [32-trash.md](/workbench/trash/) points here rather than re-specifying scratch.
...
- **Never the only copy.** `.tmp/` MUST NOT hold the sole copy of anything that matters. Material worth keeping belongs in an authored folder or, when it is a deliberate deletion, in `.trash/` ([32-trash.md](/workbench/trash/)) — which is recoverable, where `.tmp/` is not.
...
- [32-trash.md](/workbench/trash/) — the recoverable-deletion folder `.tmp/` is the throwaway counterpart to.
- [12-folders.md](/workbench/folders/) — the folder contract and the dot/no-dot rule that classes `.tmp/` as machine-local.
- [16-context.md](/workbench/context/) — the authored research store, where material worth keeping goes instead of `.tmp/`.
