...
Each of these commits is a normal commit and follows the canonical commit-message form and issue/identifier mapping defined in [17-git-workflow-and-ids.md](./17-git-workflow-and-ids.md) — scaffolding does not get a private commit convention.
...
An issue is an outward-facing artifact that a stranger can read, and it exists to coordinate work — which is why [19-internal-vs-external-communication.md](./19-internal-vs-external-communication.md) holds it to minimalism. This chapter adds the *structure* an issue takes once it is opened.
...
Crossing the prose style is the **one-language-per-artifact** rule (see [00-overview.md](./00-overview.md)): an outward document is written in a single publication language and does not mix a second working language into it. A stray label in the inward working language reads as insider noise to the outside reader, exactly as the communication chapter's leak prohibition describes.

---

## Pre-Push Quality

Before a repository's work is pushed, it passes a **pre-push checklist** — the structural counterpart to the security gate the git area runs (see [16-git-security-versioning.md](./16-git-security-versioning.md)). The checklist confirms the repository is fit to be seen and fit to be trusted:
...
The conventions above are normative, and the binding `MUST`s of each section are authored here **prose-first** as declarative requirements (the prose-first guard, [35-memo-authoring.md](./35-memo-authoring.md)): each rule's `statement` faces generation — it shapes the prompt that scaffolds a repository, fills a README block, or writes an issue — and its `check` faces the pre-push gate, where it verifies a real artifact with a ternary `PASS` / `BLOCKED` / `INCONCLUSIVE`. The structured blocks below are the machine-readable source the requirement store is **harvested** from ([23-requirements.md](./23-requirements.md)); the prose stays the document, and the blocks are its funnel tip. Most are hard structural yes/no rules whose `check` is the whole story (`grade: binary`); the few that judge a quality spectrum — method-doc completeness, diagram restraint, issue minimalism, prose readability — carry an object `grade`, and one carries the honest `todo` where a grade belongs but is not yet calibrated.
...
- [17-git-workflow-and-ids.md](./17-git-workflow-and-ids.md) — the commit-message form, the identifier-to-issue mapping, and the commit-to-issue reference this chapter's scaffolding and issues rely on.
- [19-internal-vs-external-communication.md](./19-internal-vs-external-communication.md) — the outward write-policy and leak prohibition; this chapter adds the structure, that chapter keeps the policy.
- [35-memo-authoring.md](./35-memo-authoring.md) — the deterministic-form and generated-table instinct that the README and profile-page mechanics share.
- [24-tools-registry.md](./24-tools-registry.md) — the tool catalog that the scaffolding and generation steps draw on.
- [00-overview.md](./00-overview.md) — conformance language and the one-language-per-artifact rule.
</content>
</invoke>
