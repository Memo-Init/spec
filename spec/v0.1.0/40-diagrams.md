...
|-------|-------|
| Status | Draft |
| Depends on | [35-memo-authoring.md](./35-memo-authoring.md) |
| Related | [21-human-computer-interaction.md](./21-human-computer-interaction.md), [06-memo-structure.md](./06-memo-structure.md), [00-overview.md](./00-overview.md) |

> **Informative.** This chapter offers diagram **recommendations**, not rules. Its purpose is to make the diagram variety the viewer already supports actually get used. The guidance lives here, in the open specification, rather than buried in a private skill, so that it is transparent and shared. Nothing in this chapter is a new normative requirement; the existing authoring rules in [35-memo-authoring.md](./35-memo-authoring.md) (portrait orientation, diagrams as first-class content) remain the binding ones.
...
- **Portrait is the default.** Diagrams are authored portrait (top-down) so they fit the viewer's narrow reading column without horizontal scrolling. This is the binding authoring rule from [35-memo-authoring.md](./35-memo-authoring.md); the recommendations here sit on top of it, they do not relax it.
...
- [21-human-computer-interaction.md](./21-human-computer-interaction.md) — the canonical interaction diagram, a worked reference for Mermaid style.
- [06-memo-structure.md](./06-memo-structure.md) — the `media/` folder where exported or generated assets live.
- [00-overview.md](./00-overview.md) — conformance language and the Informative/Normative marking.
