...
| Related | [02-memo-sop-entrypoint.md](./02-memo-sop-entrypoint.md), [04-input-pipeline.md](./04-input-pipeline.md), [12-rollout.md](./12-rollout.md) |
...
- **The AI never finalizes autonomously.** Finalization is exclusively developer-triggered. Experience shows the AI finalizes chronically too early; making finalization the one step that requires an explicit human trigger is a deliberate guardrail. See [11-quality-and-finalization.md](./11-quality-and-finalization.md).
- **After finalization, the AI does not ask permission for each step.** The memo was worked out jointly; once the rollout begins, everything is already agreed. The only legitimate interruption is a genuine design decision. See [12-rollout.md](./12-rollout.md).
...
The aim of all this is **moving forward** — innovation and shipped progress — not automation for its own sake. Packing the train densely is in service of pushing more worked-through topics out into the world per departure. The economics of front-loading the expensive research once and reusing it across the packed topics — the distillation economy that makes a densely packed train affordable — is described in [10-proactive-research.md](./10-proactive-research.md) (Distillate-Fan-Out).
...
- [02-memo-sop-entrypoint.md](./02-memo-sop-entrypoint.md) — the SOP that defines the guardrails.
- [04-input-pipeline.md](./04-input-pipeline.md) — the pipeline that protects long transcripts.
- [12-rollout.md](./12-rollout.md) — autonomous execution within the guardrails.
