...
There are exactly **ten** primitives: **Memo, Revision, Topic, Goal, Block, PRD, Phase, Strang, Requirement, Tool**. No other term is a primitive of this specification; new primitives MUST be introduced here, not implied elsewhere.
...
flowchart TD
    ...
    STRAND["Strang<br/>(emergent dependency closure)"]
...
> The chain is **Topic → work-package → Phase → PRD** (see [08-phases-and-prds.md](./08-phases-and-prds.md)). Strang and Requirement are **cross-cutting**, not links in that chain: a strand is the emergent dependency closure over phases ([25-strands.md](./25-strands.md)), and a requirement is a check that reaches work through its PRDs ([23-requirements.md](./23-requirements.md)). A block is a structure node identified by `B\d{3}`, not a synonym for a context block.
...
Related: [04-input-pipeline.md](./04-input-pipeline.md) (Step 3 topic extraction), [08-phases-and-prds.md](./08-phases-and-prds.md) (topics → work-packages → phases); related primitives: [Block](#block), [Phase](#phase), [PRD](#prd), [Goal](#goal).
...
A **block** is a **structure node** inside a memo, carrying its own stable identifier of the form `B\d{3}` (for example `B001`). It is **not** a synonym for a "context block": it is an addressable unit of memo structure. A block has a fence declaring `id`, `repos`, and `tags`, and a body with `### Problem`, `### Loesung`, and `### Offene Fragen`. A block's `tags` flow **additively into the requirement scope match** (meta → auto-requirements), so the structure a block declares automatically pulls in the requirements that apply to that work. A block carries **no** `strand` field: the strand is emergent and MUST NOT be declared on a block. PRDs are **derived from blocks** and take their order from the phase they belong to.
...
Related: [08-phases-and-prds.md](./08-phases-and-prds.md) (phases, `## Phase-Hints`, phase-planning lenses), [12-rollout.md](./12-rollout.md) (phase execution), [13-orchestration.md](./13-orchestration.md) (state model); related primitives: [PRD](#prd), [Strang](#strang), [Topic](#topic).
...
Related: [24-tools-registry.md](./24-tools-registry.md) (registry entries, checklist mechanic, wiki-as-a-tool), [23-requirements.md](./23-requirements.md) (tool-kind checks); related primitives: [Requirement](#requirement), [PRD](#prd), [Strang](#strang).
...
flowchart TD
    ...
    TO --> BL["Blocks (id B###): Meta + Topics<br/>Problem / Loesung / Open questions"]
...
flowchart TD
    ...
    PH -->|follow the dependency edges| STR["Strang = emergent dependency closure over phases"]
...
| Strang | Emergent dependency closure over phases; a tag, not part of the memo ID. | [25-strands.md](./25-strands.md) |
