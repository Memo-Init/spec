...
The implementation-fidelity audit is a **retrospective, fresh-context, non-blocking** measurement of how faithfully a finished memo was implemented end to end. It is the structural sibling of goal scoring ([31-goals.md](./31-goals.md)) and maintenance scoring ([33-maintenance.md](./33-maintenance.md)): a fresh context measures the **real artifact state**, distrusts every conformity report, produces a strict score object, and persists it through a deterministic CLI. It does not block a rollout and it does not finalize anything; it delivers a score and a loss table that a human or a later step reads.
...
| **B — Finalization fidelity** | `REV-N → finalized/consolidated revision` | The completeness gate ([11-quality-and-finalization.md](./11-quality-and-finalization.md)) generalized from topic to information-item granularity, plus "was anything merged incorrectly or silently absorbed?" |
| **C — Rollout coverage** | `finalized revision → PRDs → code/spec` | Every final-revision item maps to a PRD and to a real artifact change. Consumes the rollout's `conformity-report.md` and per-PRD state **distrustfully** and re-verifies at the Ist (the utilization-gate posture of [12-rollout.md](./12-rollout.md)). |
...
Hop A starts at `REV-01` deliberately: a loss that happened between the first and second revision is invisible to every gate that takes the finalized revision as its universe (the validation skills of [11-quality-and-finalization.md](./11-quality-and-finalization.md) and the PRD validation of [12-rollout.md](./12-rollout.md) both do). Hop B is where the audit and the completeness gate overlap — the gate is the special case of Hop B at topic granularity and at finalization time. Hops C and D are the part no existing mechanism performs at all.

The audit **MUST** run in a fresh context (its own empty session), never in the session that produced the work. A session that did the work reports it as done and hides the gap (the lesson behind goal scoring's never-same-session rule, [31-goals.md](./31-goals.md)). For the same reason a green `conformity-report.md` is **not** evidence: Hop C and Hop D measure the artifact, not the claim.
...
The score object is persisted through a deterministic CLI leaf, modeled on the goal-score persistence split ([31-goals.md](./31-goals.md)): the **judgement is computed by the fresh-context LLM**, the **CLI only stores the finished object**. The store is the per-memo append-only **findings channel** ([13-orchestration.md](./13-orchestration.md), the inter-agent shared memory): the fidelity result is written as a finding under a dedicated `fidelity` thread tag, append-only and dedup-keyed, so successive audits of the same memo accumulate rather than clobber. The audit never writes the memo or the revisions directly.

---

## Where It Hooks Into the Workflow

The audit is primarily **standalone and retrospective** — runnable at any time against any finished memo, exactly like goal scoring. It **MAY** additionally be offered as a **Stage-2 landing check** ([38-stage-model.md](./38-stage-model.md)): after a rollout reaches PASS, the audit runs once and delivers its score plus loss table into the landing record. Even there it stays **non-blocking** — it contributes findings, it does not hold the plane on the ground. That placement is deliberate: it is consistent with the rule that landing reports honestly rather than re-gating, and it puts the strongest available loss detector at the exact moment the memo claims to be done.
...
- [12-rollout.md](./12-rollout.md) — the rollout evaluate verifies the finalized revision against the code at rollout close; its universe begins at the finalized revision. Hops A and B see the part before that universe; Hop C re-verifies the rollout's own coverage claim distrustfully.
- [07-revisions-and-questions.md](./07-revisions-and-questions.md) — describes the contaminated-revision rescue as a manual procedure; this audit is its automation on the loss-detection side (it finds the loss; the rescue repairs it).
- [28-drift.md](./28-drift.md) — drift is a divergence between an artifact and its source **after** a memo is done; the fidelity audit is about loss **within** a memo's own implementation. A degraded fidelity score is not drift, and the two are reported separately.
- [38-stage-model.md](./38-stage-model.md) — the optional landing-check hook lives in Stage 2.
