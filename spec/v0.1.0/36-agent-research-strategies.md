# 36. Agent and Research Strategies

| Field | Value |
|-------|-------|
| Status | Draft |
| Depends on | [./13-orchestration.md](./13-orchestration.md), [./10-proactive-research.md](./10-proactive-research.md) |
| Related | [./14-agents-skills-tasks.md](./14-agents-skills-tasks.md), [./31-goals.md](./31-goals.md), [./35-memo-authoring.md](./35-memo-authoring.md), [./09-contamination-context-handover.md](./09-contamination-context-handover.md) |

This chapter collects the **strategies** for deploying agents and research effectively — the reusable patterns that decide *how* work is fanned out and *how* expensive findings are reused. It is deliberately the pattern layer, not the machinery: the orchestration mechanics (Generate → Execute → Evaluate, agent teams, parallelism dials, state and recovery) live in [13-orchestration.md](./13-orchestration.md), the research lifecycle (when to research, sub-agent spawning, where large output goes) lives in [10-proactive-research.md](./10-proactive-research.md), and the skill-versus-agent distinction lives in [14-agents-skills-tasks.md](./14-agents-skills-tasks.md). Here we name the patterns that sit above all three, so a strategy can be chosen by name rather than re-derived each time.
...
1. **Research once, deeply.** A single expensive pass (broad web/codebase sweep, many sources) produces the raw findings. Large raw output goes to the memo's `context/` ([10-proactive-research.md](./10-proactive-research.md)), never inline.
2. **Distill.** The raw findings are reduced to a **distillate** — a compact, reusable artifact that carries only what the downstream work needs. The distillate is frequently a **structured dataset** (the data payload of [35-memo-authoring.md](./35-memo-authoring.md)): one record per unit of work.
3. **Fan out cheaply.** Each record of the distillate **seeds one sub-agent** with a small, self-contained brief. The sub-agents run in parallel under the orchestration machinery ([13-orchestration.md](./13-orchestration.md)); none repeats the expensive research, because the distillate already carries it.

The payoff is that the costly part happens once and the parallel part is cheap and uniform. The same dataset that renders a generated table ([35-memo-authoring.md](./35-memo-authoring.md)) is the fan-out seed — the table is for the human reader, the records are for the sub-agents. The goal-optimization pipeline is an instance of this shape: a goal's accumulated findings are distilled into an init-transcript that seeds a follow-up memo (see [31-goals.md](./31-goals.md)).

**Mapped onto the agent-execution primitives ([14-agents-skills-tasks.md](./14-agents-skills-tasks.md)):** the one expensive research pass (step 1) runs as a single ephemeral sub-agent (type a) or a deterministic workflow (type c); the cheap fan-out (step 3) uses **type (a)** ephemeral sub-agents when there are a few records, and is the **canonical use of type (c)** — a deterministic workflow — when the records number in the dozens or hundreds (its high agent cap and script-repeatability are built for exactly that). Persistent agents (type b) are **not** the default here: the strategy depends on the fan-out workers being stateless and uniform, where type (b) fits iterative single-worker dialogue instead. This is a strategy choosing a type, not a new type.

## Fan-Out by Unit

When work decomposes into independent units — one repo, one file, one chapter, one goal — the strategy is **one agent per unit**, run in parallel, each agent scoped to exactly its unit. This keeps each context small and the work embarrassingly parallel, and it makes a missing or failed unit visible as a single gap rather than a silent omission. The degree of parallelism is a dial set by the orchestrator ([13-orchestration.md](./13-orchestration.md)); the strategy here is only the decomposition: pick a unit boundary along which the work is genuinely independent, and assign one agent to each.
...
- [10-proactive-research.md](./10-proactive-research.md) — the research lifecycle and where large research output is stored.
- [14-agents-skills-tasks.md](./14-agents-skills-tasks.md) — the skill-versus-agent distinction underneath these strategies.
- [35-memo-authoring.md](./35-memo-authoring.md) — the data payload that doubles as the fan-out seed.
- [31-goals.md](./31-goals.md) — fresh-context scoring, an instance of fresh-context evaluation.
